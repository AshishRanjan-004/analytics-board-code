...
router.post('/files', (req, res) => {
    try {
        req.body.forEach(async values => {
            let StartDateIntoMilliSecond = dateFormatter.startDateFormater(values.startDate);
            const files = new Files({
                startDate: StartDateIntoMilliSecond,
                endDate: dateFormatter.endDateFormator(StartDateIntoMilliSecond, values.endDate),
        ...
        if (retrievedFilesData.length > 0) {
            ...
            unformatedJson.forEach(value => {
                let docValueObject = {
                    ...
                    startdate: new Date(Number(value.startDate)).toISOString().substring(0, 10),
                    terminationdate: new Date(Number(value.endDate)).toISOString().substring(0, 10),
...
router.get('/projection/:entity', async (req, res) => {
    try {
        ...
        modifiedjson = JSON.stringify(modifiedjson).split(`"${req.params.entity}"`).join(`"label"`);
...
router.get('/contracts/:id', async (req, res) => {
    try {
        ...
        fs.exists('./contracts/' + retrievedFilesData[0].fileName, exists => {
            if (exists) {
                const readStream = fs.createReadStream('./contracts/' + retrievedFilesData[0].fileName, 'utf8');
                readStream.pipe(res);
            } else {
                res.status(404).json({
...
router.get("/abc", async (req, res) => {
    ...
    const disc = await Files.distinct("_id");
    res.send(disc);
